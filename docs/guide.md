...
1. Open the link you were sent. Google shows one button — **Make a copy**. Click it, and the sheet is yours, in your own Google Drive. Nothing you type ever leaves it.
2. On the **Start Here** tab, click the big **Setup / Repair** button (the **Actions** menu has the same thing). Google asks permission once, and asks narrowly: to work with **this spreadsheet only**, and to run when you're not watching — that second one is what makes repeating entries appear on their own. It cannot see anything else in your Drive. If a screen tells you the app isn't verified, that's Google's standard wording for any sheet carrying its own code; **Advanced → Continue** goes through it. Skipping this step is the one way to break Hisaab — the sheet looks perfect and quietly does nothing. You'll know it worked when the warning above the button turns into **✓ All set up!**
